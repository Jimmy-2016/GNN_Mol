...
class MoleculeDataset(Dataset):
    ...
    def process(self):
        ...
        for index, row in tqdm(self.data.iterrows(), total=self.data.shape[0]):
            ...
            sm_encoded, sm_encoded_real = smiles_encoder(row['mol'], 200)
            len = self._get_labels(np.where(sm_encoded_real == 0)[0][0] - 1)
            sm_encoded = self._get_sm_features(sm_encoded)
    ...
    def _get_sm_features(self, molfeatures):
        # molfeatures = np.asarray([molfeatures])
        return torch.tensor(molfeatures, dtype=torch.int64)
